...
export function EventoForm({ perfil }: { perfil: PerfilDeportista }) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="eyebrow">Evento o molestia especial · solo si ocurre algo puntual</div>
      <h1 className="mt-1 font-display text-2xl font-extrabold text-ink">Evento o molestia especial</h1>
      <p className="mt-2 text-sm text-slatey">Si pasó algo hoy, registralo en el momento.</p>

      <div className="card mt-6 p-5 sm:p-7">
        <Field label="¿Ocurrió alguno de estos eventos?" hint="Podés marcar más de uno">
          <MultiChips options={cfg.eventos} value={tipos} onChange={setTipos} />
        </Field>
        <Field label="Comentario breve (opcional)">
          <textarea className="input" rows={3} placeholder="Contá qué pasó, dónde y en qué momento." value={comentario} onChange={(e) => setComentario(e.target.value)} />
        </Field>
